var dappleth = (function(){ 
	...
	function setup(){
		...
		ens = ensContract.at(dappContract); //'0x112234455c3a32fd11230c42e7bccd4a84e02010'
	...
	function testRegistrar() {
		...
		apiUI.popupConfirm("Register name " + name + '.test',"Are you sure you want to register this name?").then(function(res){
			//var expire = new Date(testRegistrar.expiryTimes(web3.sha3(name)).toNumber() * 1000)

			testRegistrar.register(web3.sha3(name), apiApp.account(), 
				{from: apiApp.account(), gas: 180000, gasPrice: web3.eth.gasPrice}, 
				function(err,res){
					if(err){
						sendError(ens.address, err.message);					
					}else{
						sendMessage(ens.address, res);
						setResolver(name + ".test")
					}
	...
	function setResolver(name){
	  	var addr = apiApp.account() ; //limited only to your wallet addr

		ens.setResolver(namehash(name), publicResolver.address, 
			{from: apiApp.account(), gas: 180000, gasPrice: web3.eth.gasPrice}, 
			function(err,res){
				if(err){
					sendError(ens.address, err.message);
					...
					setAddr(name,addr);
					sendMessage(ens.address, "resolver setted! " + res);
				}
	...
	function setAddr(name, addr){
		publicResolver.setAddr(namehash(name), addr, 
			{from: apiApp.account(), gas: 180000, gasPrice: web3.eth.gasPrice}, 
			function(err,res){
				if(err){
					sendError(ens.address, err.message);
				}else{
					sendMessage(ens.address, "Addr setted! " + res);
				}
	...
	function getAddr() {
	  var name = angular.element(document.querySelector('#domain'))[0].value + ".test";

	  var node = namehash(name)
	  var resolverAddress = ens.resolver(node);
	  if(resolverAddress == '0x0000000000000000000000000000000000000000') {
	    sendMessage(ens.address, "OPS!  Not found :-(");
      	return resolverAddress; 
	  }
  	  var result = resolverContract.at(resolverAddress).addr(node);

      sendMessage(ens.address, "That's the address </br>" + "<b>" + result + "</b>");
	...
	function getOwner() {
	  var name = angular.element(document.querySelector('#domain'))[0].value + ".test";
	  var owner = ens.owner(namehash(name));
	  if(owner == "0x0000000000000000000000000000000000000000")
	      sendMessage(ens.address, " Not found :-(");
	  else
      	  sendMessage(owner, name + " it's mine!");
